    ...
    <>
      <div className="mini-player" role="region" aria-label="Now playing">
        ...
        <div className="mini-controls">
          <button className={`icon-button mini-shuffle${shuffle ? ' is-active' : ''}`} onClick={onToggleShuffle} aria-label="Toggle shuffle" aria-pressed={shuffle}><Shuffle size={16} /></button>
      ...
      {expanded && <div className="full-player-backdrop" role="dialog" aria-modal="true" aria-label="Full player" style={{ '--player-scene': artwork ? `url(${artwork})` : 'none' } as React.CSSProperties}>
        <div className="full-player">
          ...
          <div className="full-controls"><button className={`icon-button ${shuffle ? 'is-active' : ''}`} onClick={onToggleShuffle} aria-label="Toggle shuffle" aria-pressed={shuffle}><Shuffle size={19} /></button><button className="icon-button" onClick={onPrevious} aria-label="Previous track"><SkipBack size={22} fill="currentColor" /></button><button className="full-play" onClick={onPlayPause} aria-label={isPlaying ? 'Pause' : 'Play'}>{isPlaying ? <Pause size={25} fill="currentColor" /> : <Play size={25} fill="currentColor" />}</button><button className="icon-button" onClick={onNext} aria-label="Next track"><SkipForward size={22} fill="currentColor" /></button><button className={`icon-button ${repeat !== 'off' ? 'is-active' : ''}`} onClick={onToggleRepeat} aria-label="Toggle repeat" aria-pressed={repeat !== 'off'}><Repeat size={19} /></button></div>
